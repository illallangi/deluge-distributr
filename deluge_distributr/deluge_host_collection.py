...
class DelugeHostCollection(object):
    def __init__(self, config_path=None, host_filter=".*", max_torrents=100):
        self.config_path = get_app_dir("deluge") if config_path is None else config_path
        self.host_filter = (
            compile(host_filter)
            if not isinstance(host_filter, Pattern)
            else host_filter
        )
...
class AllDelugeHostsInCollectionFullException(Exception):
    def __init__(self):
        super().__init__(
            "All hosts in the collection have more than the permitted number of torrents"
        )
...
class DuplicateTorrentInCollectionException(Exception):
    def __init__(self, host, hash):
        ...
        self.hash = hash
        super().__init__(
            f"Hash {hash} on {host} is already present on another host in the collection"
        )
